...
/** Represents the data for an attribute. Each attribute value is described as a name-value pair. The name is the data
  * type, and the value is the data itself. For more information, see
...
object AttributeValue {
  ...
  def M(value: js.UndefOr[MapAttributeValue]): AttributeValue = if (value.isEmpty) AttributeValue.NULL(true)
  else js.Dynamic.literal("M" -> value.asInstanceOf[js.Any]).asInstanceOf[AttributeValue]
  def M(value: Option[MapAttributeValue]): AttributeValue =
    if (value.isEmpty) AttributeValue.NULL(true) else js.Dynamic.literal("M" -> value.get).asInstanceOf[AttributeValue]
  ...
  def NS(value: NumberSetAttributeValue): AttributeValue =
